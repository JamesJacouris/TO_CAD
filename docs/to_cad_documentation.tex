...
\vspace{1cm}
{\Large Converting Mesh-Based TO Geometries to\\Parametric CAD Models\par}
\vspace{2cm}
...
\textbf{TO\_CAD} converts mesh-based topology optimisation (TO) geometries into
editable parametric CAD models (beams, plates, and fused solid bodies) ready
for FreeCAD.  The pipeline chains four stages:
...
\begin{tikzpicture}[
    ...
    every node/.style={
        draw, rounded corners=2pt, font=\scriptsize\ttfamily,
        fill=blue!5, minimum height=7mm, minimum width=28mm,
        inner sep=3pt, align=center
    },
    orch/.style={ellipse, fill=blue!15, minimum width=30mm},
    hybrid/.style={fill=green!10},
    optional/.style={fill=orange!10},
    edge/.style={-{Stealth[length=3pt]}, semithick},
    dashedge/.style={-{Stealth[length=3pt]}, semithick, dashed},
    grouplabel/.style={draw=none, fill=none, font=\scriptsize\sffamily\bfseries,
                       minimum height=0mm, minimum width=0mm, text=gray!70!black},
]

% ── Column 1: Orchestrators ──
\node[orch] (runpipe) {run\_pipeline.py};
\node[orch, below=14mm of runpipe] (runtop) {run\_top3d.py};

% ── Column 2: Stage 0 + mesh import ──
\node[right=22mm of runtop] (top3d) {optimization.top3d};
\node[optional, below=10mm of top3d] (meshvox) {mesh\_import.\\mesh\_voxelizer};

% ── Column 3: Stage 1 reconstruction ──
\node[right=22mm of runpipe, yshift=10mm] (recon) {baseline\_yin.\\reconstruct};
\node[below=7mm of recon] (thin) {baseline\_yin.\\thinning};
\node[below=7mm of thin] (topo) {baseline\_yin.\\topology};
\node[below=7mm of topo] (graphm) {baseline\_yin.\\graph};
\node[below=7mm of graphm] (postproc) {baseline\_yin.\\postprocessing};
\node[hybrid, below=7mm of postproc] (plateext) {baseline\_yin.\\plate\_extraction};
\node[hybrid, below=7mm of plateext] (jointcr) {baseline\_yin.\\joint\_creation};

% ── Column 4: Stages 2--3 optimisation ──
\node[right=22mm of recon] (sizeopt) {optimization.\\size\_opt};
\node[below=10mm of sizeopt] (layoutopt) {optimization.\\layout\_opt};
\node[below=10mm of layoutopt] (fem) {optimization.fem};

% ── Column 5: Support modules ──
\node[right=22mm of sizeopt] (problems) {problems};
\node[optional, below=10mm of problems] (symm) {optimization.\\symmetry};
\node[optional, below=10mm of symm] (spline) {curves.spline};

% ── Column group labels ──
\node[grouplabel, above=4mm of runpipe] {Orchestrators};
\node[grouplabel, above=4mm of top3d] {Stage 0};
\node[grouplabel, above=4mm of recon] {Stage 1};
\node[grouplabel, above=4mm of sizeopt] {Stages 2--3};
\node[grouplabel, above=4mm of problems] {Support};

% ── Edges: solid = required ──
\draw[edge] (runpipe) -- (runtop);
\draw[edge] (runtop) -- (top3d);
\draw[edge] (runpipe.east) to[out=10,in=180] (recon.west);
\draw[edge] (recon) -- (thin);
\draw[edge] (thin) -- (topo);
\draw[edge] (recon.east) to[out=-10,in=180] (graphm.west);
\draw[edge] (recon.east) to[out=-15,in=180] (postproc.west);
\draw[edge] (runpipe.east) to[out=5,in=170] (sizeopt.west);
\draw[edge] (runpipe.east) to[out=0,in=170] (layoutopt.west);
\draw[edge] (sizeopt) -- (fem);
\draw[edge] (layoutopt) -- (fem);
\draw[edge] (sizeopt.east) to[out=0,in=180] (problems.west);
\draw[edge] (layoutopt.east) to[out=0,in=190] (problems.west);
\draw[edge] (runpipe.east) to[out=5,in=160] (problems.west);

% ── Edges: dashed = hybrid / optional ──
\draw[dashedge] (recon.east) to[out=-20,in=180] (plateext.west);
\draw[dashedge] (recon.east) to[out=-25,in=180] (jointcr.west);
\draw[dashedge] (runpipe.south) to[out=-70,in=180] (meshvox.west);
\draw[dashedge] (layoutopt.east) to[out=-10,in=180] (spline.west);
\draw[dashedge] (runpipe.east) to[out=2,in=170] (symm.west);

\end{tikzpicture}%
}% end resizebox
\caption{Pipeline module dependencies.  Dashed edges indicate hybrid-only or
optional paths.  Green-filled modules are hybrid-only; orange-filled are optional.}
...
\begin{figure}[htbp]
\centering
\input{figures/roof_problem_setup}
...
\begin{figure}[htbp]
\centering
\input{figures/frame_supported_beam_setup}
